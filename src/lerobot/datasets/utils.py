...
def hw_to_dataset_features(
    ...
    # Camera/image features are 3D tuples (H, W, C). Other tuple shapes (e.g. (15,))
    # are treated as vector numeric features.
    cam_fts = {
        key: shape
    ...
    # Preserve explicit vector observation features (e.g. "observation.wowskin": (15,))
    # as standalone dataset features instead of converting them to image/video features.
    if prefix == OBS_STR:
...
def build_dataset_frame(
    ...
    for key, ft in ds_features.items():
        if key in DEFAULT_FEATURES or not key.startswith(prefix):
            ...
            # Aggregated vector feature with named scalar elements (e.g. observation.state)
            if "names" in ft and ft["names"] is not None:
                frame[key] = np.array([values[name] for name in ft["names"]], dtype=np.float32)
            # Standalone vector feature already present in values (e.g. observation.wowskin)
            else:
                source_key = key.removeprefix(f"{prefix}.")
                if source_key in values:
                    frame[key] = np.asarray(values[source_key], dtype=np.float32)
                else:
